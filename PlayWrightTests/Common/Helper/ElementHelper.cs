...
namespace PlaywrightTests.Common.Helper
{
    public class ElementHelper
    {
        ...
        public static IFrameLocator GetIFrameLocator(IPage? page, string iframeName)
        {
            ...
            {
               // throw new CustomLogException($"{CustomExceptionPrefix.CodeError_Element_Load_Failed} IFrameName is null");
            }
          ...
          //  LogHelper.Info($"Iframe name: {iframeName}, check if it's loaded...");
            ...
            //LogHelper.Error($"{CustomExceptionPrefix.CodeError_Element_Load_Failed} IFrame:{iframeName} load time out");
            //throw new CustomLogException($"{CustomExceptionPrefix.CodeError_Element_Load_Failed} IFrame:{iframeName} load time out");
        ...
        public static async Task<ILocator> GetByClassAndHasTextAsync(IPage? page, string className, string hasText, string? hasNotText = null, bool exact = false, bool waitUntilElementExist = true)
        {
        ...
        public static async Task<ILocator> GetByClassAndHasTextAsync(IFrameLocator iframe, string className, string hasText, string? hasNotText = null, bool exact = false, bool waitUntilElementExist = true)
        {
        ...
        public static async Task<ILocator> GetByClassAndAriaLableAsync(IPage? page, string className, string arialabel, bool exact = false, bool ariaLabelExact = true, bool IsStrongWait = true)
        {
            try
            {
                arialabel = arialabel.Replace("'", "\\'");
        ...
        public static async Task<ILocator> IsExistAsync(ILocator? locator, int retry = 150, bool isNeedSleep = true)
        {
            if (locator == null)
            {
            ...
            while (true)
            {
                try
                {
                    ...
                    if (locatorCount != 0)
                    {
                        LogHelper.Info($"Try `{count}` check if current locator is exist...`Done`");
                        LogHelper.Info($"Current Locator {locator.ToString()}");
                        if (isNeedSleep)
                        {
                            Thread.Sleep(200);
                        }
                    ...
                    else
                    {
                        ...
                        if (count >= retry)
                        {
                            ...
                            throw new CustomLogException(CustomExceptionPrefix.CodeError_Element_Load_Failed + ":" + locator.ToString());
                        }
                        if (isNeedSleep)
                        {
                            Thread.Sleep(200);
        ...
        public static async Task<bool> IsEnabledAsync(ILocator element, int waitTime = 200, int waitTimes = 150)
        {
            try
            {
                ...
                if (element == null)
                {
                    throw new CustomLogException("element not is null or empty...");
                }
            ...
            catch (Exception ex)
            {
                ...
                throw new CustomLogException("Element load failed: element enabled failed...", ex);
            }
        }
        #endregion
    }
