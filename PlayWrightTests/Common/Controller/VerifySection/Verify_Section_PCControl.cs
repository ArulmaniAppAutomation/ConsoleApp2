...
{
    public class Verify_Section_PCControl : BaseController, IVerifySection
    {
        private string header;
        private string? node;
        public Verify_Section_PCControl(IPage? page,string header, string? frameName, EnumHelper.Language language, ILocator? parentLocator = null, string? node = null) : base(page, frameName,language, parentLocator)
        {
            this.header = header;
            this.node = node;
        }
        ...
        {
            string? ActualSettingValue = await ControlHelper.GetTextByClassAndTextKeywordAsync(SectionLocator, "fxc-weave-pccontrol fxc-section-control fxc-base msportalfx-customHtml msportalfx-form-formelement", settingName, nth);
            LogHelper.Info($"Settings \"{settingName}\" expect value: {expectSettingValue}");
            Assert.That(ActualSettingValue.Contains(expectSettingValue), Is.True);
        ...
        {
            var result = await ControlHelper.ClickByClassAndHasTextAsync(SectionLocator, "fxc-weave-pccontrol fxc-section-control fxc-base msportalfx-customHtml msportalfx-form-formelement", settingName, 0, isClick: false, waitUntilElementExist: false, hasNotText: hasNotText);
        ...
        private async Task<ILocator> GetSectionLocatorAsync()
        {
            ...
            try
            {
                ...
                await locator.Nth(0).ScrollIntoViewIfNeededAsync();
            }
            catch
            {
                locator = this.CurrentIPage.Locator("body");
            ...
            if (node != null)
            {
                var nodeLocator = await ControlHelper.GetLocatorByClassAndHasTextAsync(locator.Nth(0), "fxs-portal-border fxc-accordion-section fxc-accordion-section-expanded", node, 0);
                return nodeLocator;
            }
            else
            {
