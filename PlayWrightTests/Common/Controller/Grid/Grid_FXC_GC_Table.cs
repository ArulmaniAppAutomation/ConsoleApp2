...
{
    public class Grid_FXC_GC_Table : GridBase, IGrid
    {
        private ILocator? Grid_Locator { get { return GetGridLocatorAsync().Result; } }
        private IConfirmDialog confirmDialog;
        private IContextMenu contextMenu;
        public Grid_FXC_GC_Table(IPage? page, string? frameName, EnumHelper.Language language, ILocator? parentLocator = null) : base(page, frameName, language, parentLocator)
        {
        ...
        { throw new NotImplementedException(); }
        public async Task RemoveRowByRowHeaderAsync(string RowHeader)
        {
            ...
            await contextMenu.CLickSpecialNameMenuItemAsync("Remove");
        }
        ...
        private async Task<ILocator> GetGridLocatorAsync()
        {
            ...
            {
                return await ControlHelper.GetLocatorByClassAsync(this.ParentLocator, "fxc-gc-table", 0);
            }
            return await ControlHelper.GetLocatorByClassAsync(this.CurrentIPage, "fxc-gc-table", 0, iFrameName: this.CurrentIFrameName);
        }
