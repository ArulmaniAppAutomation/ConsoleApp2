...
{
    public class SelectFromGridBySearchUtils : BaseCommonUtils
    {
        ...
        {
            var complementaryLocator = await ControlHelper.GetByRoleAndHasTextAsync(_page, AriaRole.Complementary, title, 0, iFrameName: null);
        ...
        private async Task SelectBySearchWithKeyAndValuesAsync(string title, string searchBoxLabel, string key, List<string> values, bool isExpandAllNode = true, string? actualName = "")
        {
            ...
            await ControlHelper.ClickByClassAndHasTextAsync(_page, "ext-search-button fxc-base fxc-simplebutton", "Search", 0, iFrameName: null);
           // CommonOperations.WaitMiddleTime();
            ...
            while (true)
            {
                ...
                await ControlHelper.ClickByTreeItemRoleAndNameAsync(_page, name, 0, iFrameName: null);
                //wait for loading disappear , such as : Defender - Loading...
                int i = 0;
                while (true)
                {
                    var locator = await ControlHelper.GetLocatorByClassAndHasTextAsync(_page, "azc-treeView-node-content", $"{key} - Loading...", 0, null, waitUntilElementExist: false);
            ...
            foreach (var setting in values)
            {
                ...
                if (setting.Contains("|") && !setting.Contains("**|"))
                {
                    ...
                    locator = await ControlHelper.GetLocatorByClassAndHasTextAsync(_page, "fxc-gc-cell", settingArray[0], 0, hasNotText: settingArray[1]);
                }
                else if (setting.Contains("**|"))
                {
                    locator = await ControlHelper.GetLocatorByClassAndHasTextAsync(_page, "fxc-gc-cell", setting.Replace("**|", "|"), 0);
                }
                else
                {
                    locator = await ControlHelper.GetLocatorByClassAndHasTextAsync(_page, "fxc-gc-cell", setting, 0);
                }
            ...
            await ClickCloseContentBtnAsync(_page, "Settings picker");
        }
        ...
        {
            var pannelContainerLocator = await ControlHelper.GetLocatorByClassAndHasTextAsync(_page, titleContainerClass, title, 0, iframeName: iFrameName);
            ...
            await ControlHelper.ClickByClassAndHasTextAsync(_page, "ms-Button ms-Button--primary", "Save", 0, iFrameName: iFrameName);
